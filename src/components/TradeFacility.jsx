...
function BuyerTrackerCard({ txn, idx }) {
  ...
  return (
    ...
    >
      ...
      {/* Trade item the buyer is giving (shown to buyer) */}
        ...
        {currentTransactions.length === 0 ? (
          ...
          <div className={styles.list}>
              ...
              return (
                <div key={txn.id} className={styles.card} style={{ animationDelay: `${idx * 0.06}s` }}>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    <div className={styles.metaRow}>
                      <span className={styles.metaPrice}>R{formatPrice(txn.agreedPrice ?? txn.listing?.price)}</span>
                      <span className={styles.metaDot}>·</span>
                      <span className={styles.metaItem}>
                    ...
                    {isTrade && !txn.buyerBookingId && txn.type === 'trade' && (
            ...
            {activeTab === "buyer" && buyerTransactions.map((txn, idx) => {
              ...
              // For trade transactions in buyer tab, also show a "book your drop-off" CTA
              if (isTrade) {
                return (
                  <div key={txn.id}>
                    <BuyerTrackerCard txn={txn} idx={idx} />
                    {/* CTA if buyer hasn't booked their slot yet */}
                    {canBookBuyerDropOff(txn) && (
                      <div style={{
                        margin: '-4px 0 8px', padding: '10px 14px',
                        background: '#ede9fe', border: '1px solid #c4b5fd',
                        borderRadius: '0 0 12px 12px', display: 'flex',
                        alignItems: 'center', justifyContent: 'space-between', gap: 10,
                      }}>
                        <p style={{ margin: 0, fontSize: '0.78rem', color: '#5b21b6', fontWeight: 600 }}>
                          🔄 Book your trade item drop-off slot
                        </p>
                        <button
                    ...
                    {hasBuyerDropOffBooked(txn) && (
                      <div style={{
                        margin: '-4px 0 8px', padding: '8px 14px',
                        background: '#f0fdf4', border: '1px solid #86efac',
                        borderRadius: '0 0 12px 12px',
                        fontSize: '0.75rem', color: '#166534', fontWeight: 600,
                      }}>
                        ✓ Your trade drop-off is booked: {txn.buyerDropOffDate} · {txn.buyerDropOffTimeSlot}
                      </div>
                    )}
                  </div>
                );
              ...
              return <BuyerTrackerCard key={txn.id} txn={txn} idx={idx} />;
            })}
