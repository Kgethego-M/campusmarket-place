            ...
            {byType.map(row => (
              <tr key={row.type} style={{ borderBottom: "1px solid #f8fafc" }}>
                <td style={{ padding: "11px 14px", fontWeight: 600, color: "#1e293b" }}>
                  <i className={`fas ${row.type === "user" ? "fa-user" : row.type === "listing" ? "fa-store" : "fa-star"}`} style={{ marginRight: 6, color: row.type === "user" ? "#94a3b8" : row.type === "listing" ? "#6AA6DA" : "#f59e0b" }} />
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAdRevenue = async () => {
      try {
        const snapshot = await getDocs(collection(db, "adRevenue"));
        const total = snapshot.docs.reduce((sum, doc) => sum + (doc.data().amount || 0), 0);
        setAdRevenueTotal(total);
      } catch (err) {
        console.error("Failed to fetch ad revenue:", err);
      }
    };
    fetchAdRevenue();
  }, []);
