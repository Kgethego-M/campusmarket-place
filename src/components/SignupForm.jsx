...
// ── Email-type checks (mirrors LoginForm) ─────────────────────────────────────
  ...
  async function handleSignup(selectedRole) {
    ...
    try {
      ...
      if (selectedRole === 'student' && !isStudentEmail(email)) {
        await signOut(auth);
        setShowPopup(false);
        show(`Student sign-up requires a @students.wits.ac.za or approved student email. You used: ${email}`, true);
        return;
      }
      ...
      if (selectedRole === 'staff' && !isStaffEmail(email)) {
        await signOut(auth);
        setShowPopup(false);
        show(`Staff sign-up requires a @wits.ac.za or approved staff email. You used: ${email}`, true);
        return;
      }
      ...
      if (selectedRole === 'admin' && !isAdminEmail(email)) {
        await signOut(auth);
        setShowPopup(false);
        show(`Admin sign-up requires an approved admin email. You used: ${email}`, true);
        return;
      }
  ...
  return (
    <>
      ...
      <div className="form-card">
        ...
        <p className="form-subtitle">Sign up with your Wits Google account</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button className="role-btn" disabled={loading} onClick={() => handleSignup('student')}>
          ...
          <button className="role-btn" disabled={loading} onClick={() => handleSignup('staff')}>
          ...
          <button className="role-btn" disabled={loading} onClick={() => handleSignup('admin')}>
      ...
      {showPopup && (
        <div style={popupStyles.overlay}>
          <div style={popupStyles.popup}>
            ...
            <p style={popupStyles.text}>Creating your account…</p>
          </div>
        </div>
      )}
    </>
